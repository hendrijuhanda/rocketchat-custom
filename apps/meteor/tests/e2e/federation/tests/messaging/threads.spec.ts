	...
	test.describe('Messaging - Threads', () => {
		...
		test.describe('Send "Special" messages', () => {
			...
			test('expect to send an audio message from Server A to Server B', async ({ page }) => {
				...
				expect(await (await poFederationChannelServer2.content.getLastFileThreadMessageByFileName('Audio record.mp3')).innerText()).toEqual(
					'Audio record.mp3',
				);
				...
				expect(await (await poFederationChannelServer1.content.getLastFileThreadMessageByFileName('Audio record.mp3')).innerText()).toEqual(
					'Audio record.mp3',
				);
			});
