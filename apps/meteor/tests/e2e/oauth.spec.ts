...
test.describe('OAuth', () => {
	...
	test('Login Page', async ({ page, api }) => {
		await test.step('expect OAuth button to be visible', async () => {
			expect((await setSettingValueById(api, 'Accounts_OAuth_Google', true)).status()).toBe(200);
		...
		await test.step('expect Custom OAuth button to be visible', async () => {
			expect((await setSettingValueById(api, 'Accounts_OAuth_Custom-Test', true)).status()).toBe(200);
			await page.waitForTimeout(5000);
			await page.goto('/home');
		...
		await test.step('expect OAuth button to not be visible', async () => {
			expect((await setSettingValueById(api, 'Accounts_OAuth_Google', false)).status()).toBe(200);
			await page.waitForTimeout(5000);
		...
		await test.step('expect Custom OAuth button to not be visible', async () => {
			expect((await setSettingValueById(api, 'Accounts_OAuth_Custom-Test', false)).status()).toBe(200);
			await page.waitForTimeout(5000);
